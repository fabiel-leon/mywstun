const mws = require('websocket').client;
...
const extend = function extend(dest, source) {
  for (const prop in source) {
    dest[prop] = source[prop];
  }
};

const eventEmitterListenerCount = require('events').EventEmitter.listenerCount || function (emitter, type) {
...
mws.prototype.connect = function (requestUrl, protocols, origin, headers, extraRequestOptions) {
  ...
  if (this.secure) {
    for (const key in self.config.tlsOptions) {
