...
(function () {
  ...
  module.exports = wst_server = (function () {
    ...
    wst_server.prototype.start = function (port) {
      if (https_flag == 'true') { console.log(`[SYSTEM] - ${new Date()} - WS Tunnel Server starting on: wss://localhost:${port} - CERT: \n${this.s4t_cert}`); } else { console.log(`[SYSTEM] - ${new Date()} - WS Tunnel Server starting on: ws://localhost:${port}`); }
