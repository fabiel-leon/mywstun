  ...
  module.exports = bindSockets = function (wsconn, udpconn, port, address) {
    ...
    wsconn.on('message', (message) => {
      if (message.type === 'utf8') {
        ...
        console.clear();
        // console.log('\rRECEIVED WS MESSAGE');
        udpconn.send(message.binaryData, port || crinfo.port, address || crinfo.address, (err) => {
          // console.log('SENDING UDP MESSAGE', port || crinfo.port, address || crinfo.address);
          if (err) {
            console.error(err, 'socket paused');
            wsconn.socket.pause();
    ...
    udpconn.on('message', (buffer, rinfo) => {
      // console.clear();
      // console.log('RECEIVED UDP MESSAGE', rinfo.address, rinfo.port);
      crinfo = rinfo;
      // console.log('[SYSTEM] - ' + (new Date()) + ' --> TCP data received:\n\n\n' + buffer + "\n\n"); //console.log(JSON.stringify(buffer));
      // console.log('SENDING WS MESSAGE', port || crinfo.port, address || crinfo.address);
